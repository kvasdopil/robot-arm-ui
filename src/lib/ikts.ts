...
function angleBetween3D(a: Vec3, b: Vec3, ref: Vec3) {
  ...
  const sign = Math.sign(ref[0] * cross[0] + ref[1] * cross[1] + ref[2] * cross[2]);
...
export function extractYawPitchDegrees(result: RobotArmIkResult): {
  ...
  const sDirX = sdx / sl; // eslint-disable-line @typescript-eslint/no-unused-vars
  const sDirZ = sdz / sl; // eslint-disable-line @typescript-eslint/no-unused-vars
  ...
  const projOntoPlane = (v: Vector3, axis: Vector3) =>
    v
      .clone()
  ...
  if (forearm && ankle2) {
    ...
    const projRef2 = refWorld2
      .clone()
      .sub(a2Axis.clone().multiplyScalar(refWorld2.dot(a2Axis)))
      .normalize();
    const projFore2 = fDir
      .clone()
      .sub(a2Axis.clone().multiplyScalar(fDir.dot(a2Axis)))
      .normalize();
    ...
    return {
      ...
      forearmDeg: Math.max(-135, Math.min(135, forearmDeg)),
    };
  }
